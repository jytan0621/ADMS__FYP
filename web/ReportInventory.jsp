<%-- 
    ...
    Created on : May 26, 2026, 8:59:34 PM
    Author     : User
--%>
...
<%
    ...
    List<Map<String, Object>> inventory = (List<Map<String, Object>>) request.getAttribute("reportInventory");
...
<html>
<head>
    <title>ADMS - Shelter Allocation Balance</title>
    ...
    <style>
        ...
        @media print {
            ...
            .card::before {
                content: "Shelter Allocation Balance Report";
                display: block;
                font-size: 22px;
                font-weight: bold;
                text-align: center;
        ...
        .btn-action { padding: 8px 18px; border: none; border-radius: 6px; font-weight: bold; cursor: pointer; display: inline-flex; align-items: center; gap: 8px; font-size: 13px; color: white;}
        ...
        .badge-warn { background: #fee2e2; color: #991b1b; padding: 4px 8px; border-radius: 4px; font-size: 11px; font-weight: bold; }
...
<body>
    ...
    <div class="main-content">
        <div class="toolbar">
            <h2 style="margin:0 0 16px 0; color: #1e293b;"><i class="fas fa-boxes-stacked" style="color:#0b5ea8;"></i> Shelter Allocation Balance Report</h2>
            <form action="reportInventory" method="GET" class="filter-container">
                ...
                <div class="filter-group">
                    ...
                    <button type="submit" class="btn-action btn-filter"><i class="fas fa-search"></i> Load Balances</button>
                </div>
                <% } else { %>
        ...
        <div class="card">
            <% if (promptSelect) { %>
                <div class="prompt-box">
                    <i class="fas fa-box-open" style="font-size: 40px; margin-bottom: 15px; color:#cbd5e1;"></i>
                    <h3>Awaiting Selection</h3>
                    <p>Please select a shelter to view its specific resource allocations and balances.</p>
                </div>
            <% } else { %>
            <table id="target-table">
                <thead>
                    <tr>
                        ...
                        <th style="text-align:center;">Total Received (In)</th>
                        <th style="text-align:center;">Total Distributed (Out)</th>
                ...
                <tbody>
                    <% if(inventory != null && !inventory.isEmpty()) { 
                        ...
                        for(Map<String, Object> item : inventory) { 
                            boolean isShort = (Boolean) item.get("isInsufficient");
                    %>
                    <tr>
                        <td style="font-weight: bold; color: #64748b;"><%= count++ %></td>
                        <td style="font-weight: 600;"><%= item.get("itemName") %></td>
                        <td style="color:#64748b; font-size:12px;"><%= item.get("distType") %></td>
                        <td style="text-align:center;"><%= item.get("totalAllocated") %></td>
                        <td style="text-align:center;"><%= item.get("distributed") %></td>
                        <td style="text-align:center; font-weight:bold; font-size: 15px;"><%= item.get("remainingBalance") %></td>
                        <td style="text-align:center;">
                            <% if(isShort) { %>
                                <span class="badge-warn">Shortage: <%= item.get("shortage") %> Needed</span>
                            <% } else { %>
                                <span style="color:#10b981; font-weight:bold;"><i class="fas fa-check-circle"></i> Sufficient</span>
                            <% } %>
                        </td>
                    </tr>
                    <% } } else { %>
                    <tr><td colspan="7" style="text-align:center; padding:30px;">No allocations found for this shelter.</td></tr>
                    <% } %>
                </tbody>
            </table>
            <% } %>
...
<script>
        function exportCSV() {
            ...
            var filename = "Shelter_Allocation_Balance.csv";
